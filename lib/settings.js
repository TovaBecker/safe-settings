...
`
      ...
      const newComment = await this.github.rest.issues.createComment({
        owner,
        repo,
      ...
      if (env.PR_COMMENT_DEDUPE_ENABLED === 'true') {
        try {
          ...
          const newCommentUserId = newComment.data.user?.id
          const candidates = newCommentUserId === undefined
            ? []
            : comments.filter(comment =>
              comment.id < newComment.data.id &&
              comment.node_id &&
              comment.user?.id === newCommentUserId &&
              comment.body?.trim().startsWith('#### :robot: Safe-Settings config changes detected:')
            )

          let staleComments = candidates
          if (candidates.length > 0) {
            const NODE_QUERY_BATCH_SIZE = 100
            const alreadyMinimized = new Set()
            for (let i = 0; i < candidates.length; i += NODE_QUERY_BATCH_SIZE) {
              const batch = candidates.slice(i, i + NODE_QUERY_BATCH_SIZE)
              const nodeStates = await this.github.graphql(
                `query($ids: [ID!]!) {
                  nodes(ids: $ids) {
                    ... on IssueComment {
                      id
                ...
                { ids: batch.map(candidate => candidate.node_id) }
              )
              for (const node of nodeStates.nodes || []) {
                if (node?.isMinimized) {
                  alreadyMinimized.add(node.id)
                }
              }
            }
            staleComments = candidates.filter(candidate => !alreadyMinimized.has(candidate.node_id))
          }
          ...
          for (const staleComment of staleComments) {
            try {
              await this.github.graphql(
                ...
                { id: staleComment.node_id }
              )
            } catch (e) {
              this.log.debug(`Failed to minimize a previous Safe-Settings comment: ${e}`)
            }
          }
        } catch (e) {
          this.log.debug(`Failed to list previous Safe-Settings comments: ${e}`)
        }
      }
    }
